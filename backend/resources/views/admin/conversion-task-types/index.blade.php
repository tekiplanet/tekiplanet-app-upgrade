...
<div class="container px-6 mx-auto">
    ...
    <div class="mb-6 bg-white rounded-lg shadow-md dark:bg-gray-800 p-4">
        <form action="{{ route('admin.conversion-task-types.index') }}" method="GET" class="flex flex-col md:flex-row gap-4">
            <div class="flex-1">
                <input type="text" 
                       name="search" 
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <form action="{{ route('admin.conversion-task-types.destroy', $type) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-600 hover:text-red-800" onclick="return confirm('Delete this task type?')">
                                            Delete
                                        </button>
                                    </form>
                                </div>
...
@include('admin.conversion-task-types._form_modal')
...
function openCreateModal() {
    ...
    document.getElementById('taskTypeModal').classList.remove('hidden');
}
...
function openEditModal(type) {
    ...
    form.action = `/admin/conversion-task-types/${type.id}`;
    form.querySelector('input[name="_method"]').value = 'PUT';
    document.getElementById('modalTitle').textContent = 'Edit Task Type';
    document.getElementById('taskTypeModal').classList.remove('hidden');
}
